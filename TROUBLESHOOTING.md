...
- **It hears me but answers slowly**: the default is **Haiku** (`model` in backtalk.json). Say **"switch to haiku"** to go back to the fast tier, or edit `model` / `balanced_model` / `deep_model` for your defaults. Full Opus/Sonnet tiers are slower on purpose. Model ids must be the FULL id from Claude Code, never a bare alias.
...
- **While paused, nothing reaches the agent**: that is intentional. Say "resume" (hold the talk key and say it, or type it in the terminal), press **Fn+Control**, or use the control panel to listen again. Hands-free open mic is off during pause so room audio cannot trigger replies.
- **Visual controls (pause / model)**: while the voice line runs, open http://127.0.0.1:8792/ or double-click `Open Control.command`. On ai-visualizer faces, move the mouse — buttons appear bottom-right (Pause, Resume, Fast, Deep, Toggle). The face reads the control URL from `.voice_control_url` on the signal bus (written when voice starts).
- **Face stuck idle but voice works**: run `./setup.sh --yes` to re-wire `bus_dir`, or check `ai-visualizer.json` → `bus_dir` points at your backtalk folder. Logs: `~/.local/share/backtalk/logs/visualizer.log`.
- **Closing Stack terminal killed the face (old shortcuts)**: re-run `./make-desktop-launcher.sh` — Stack now uses detached `start-visualizer.sh`.
- **Fn+Option / Fn+Control hotkeys do nothing**: same permission as push-to-talk — Terminal needs Input Monitoring. Some keyboards never expose Fn to apps; set `"hotkeys": {"toggle_model": "ctrl+option+m", "toggle_pause": "ctrl+shift+p"}` in backtalk.json and restart. Set `"toggle_pause": ""` to disable the pause hotkey.
