...
- **Visual controls (pause / model)**: while the voice line runs, open http://127.0.0.1:8792/ or double-click `Open Control.command`. On ai-visualizer faces, move the mouse — buttons appear bottom-right (Pause, Resume, Fast, Deep, Toggle). The face must use an updated ai-visualizer that reads the control API.
